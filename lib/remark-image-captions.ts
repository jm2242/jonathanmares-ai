...
export function remarkImageCaptions() {
  return (tree: Root) => {
    ...
    visit(tree, "paragraph", (paragraph: Paragraph, _index, parent) => {
      ...
      const emIndex = paragraph.children.findIndex(
        (child, idx) => idx > imageIndex && child.type === "emphasis"
      );
    ...
    visit(tree, "image", (image: Image, _index, parent) => {
      if (!parent || !Array.isArray(parent.children)) return;
      ...
      for (let i = imageIndex + 1; i < parent.children.length && i < imageIndex + 5; i++) {
        ...
        if (nextNode && nextNode.type === "paragraph") {
          ...
          if (paragraph.children.length === 1 && paragraph.children[0].type === "emphasis") {
    ...
    replacements.reverse().forEach(({ paragraph, imageIndex, parent }) => {
      ...
      const emNode = paragraph.children.find(
        (child) =>
          child.type === "emphasis" &&
